...
class MarketReportService(
    ...
    /**
     * Slack 메시지를 한국 개인 투자자가 빠르게 훑어볼 수 있는 형태로 정리한다.
     * 숫자는 그대로 보여주되 해석과 액션은 짧고 보수적으로 제시한다.
     */
    ...
    private fun buildInterpretations(
        ...
        sp?.changePct?.let { p ->
            if (p <= BigDecimal("-1.00")) {
                out.add("미국 대형주 지수가 약세라면 오늘 장 초반 변동성 확대에 대비하는 편이 좋습니다.")
            }
            if (p >= BigDecimal("1.00")) {
                out.add("미국 대형주 지수가 강세여서 위험자산 선호 분위기가 이어질 수 있습니다.")
            }
        }

        nq?.changePct?.let { p ->
            if (p <= BigDecimal("-1.00")) {
                out.add("나스닥 약세가 크면 성장주와 고밸류 종목의 변동성을 더 주의해서 볼 필요가 있습니다.")
            }
            if (p >= BigDecimal("1.00")) {
                out.add("나스닥 강세는 기술주 심리 개선 신호로 해석할 수 있습니다.")
            }
        }

        y10?.value?.let { v ->
            if (v >= BigDecimal("4.00")) {
                out.add("미국 10년물 금리가 4% 이상이면 성장주 밸류에이션 부담이 커질 수 있습니다.")
            }
            if (v <= BigDecimal("3.50")) {
                out.add("미국 10년물 금리가 낮은 구간이면 성장주에 우호적인 환경으로 볼 수 있습니다.")
            }
        }

        fx?.value?.let { v ->
            if (v >= BigDecimal("1400")) {
                out.add("원달러 환율이 1400원 이상이면 환차손과 변동성 관리가 더 중요해집니다.")
            }
            if (v <= BigDecimal("1300")) {
                out.add("원달러 환율이 1300원 아래면 환 부담이 완화된 구간으로 볼 수 있습니다.")
            }
        }
    ...
    private fun buildActions(
        ...
        val bigMove = listOfNotNull(sp?.changePct?.abs(), nq?.changePct?.abs())
            .any { it >= BigDecimal("1.00") }
        if (bigMove) {
            out.add("보유 종목과 ETF의 변동성, 손절 기준, 분할매수 기준을 다시 확인합니다.")
        }

        fx?.value?.let { v ->
            if (v >= BigDecimal("1400")) {
                out.add("환율이 높으면 달러 매수와 환전은 분할 접근이 유리한지 점검합니다.")
            }
        }

        y10?.value?.let { v ->
            if (v >= BigDecimal("4.00")) {
                out.add("금리 부담 구간에서는 성장주 비중과 현금 비중을 함께 점검합니다.")
            }
        }

        out.add("오늘 예정된 경제지표와 주요 이벤트를 확인하고 매매 근거를 짧게 기록합니다.")
    ...
    private fun buildGeminiPrompt(runDate: LocalDate, metrics: List<Metric>): String {
        val lines = metrics.joinToString("\n") { m ->
            ...
            val change = m.change?.setScale(2, RoundingMode.HALF_UP)?.toPlainString() ?: "N/A"
            "${m.name}: value=${m.value}, change=$change, changePct=$pct"
        }

        return """
        너는 한국 개인 투자자를 위한 아침 시장 브리핑 도우미다.
        아래 데이터를 기반으로 짧고 보수적으로 해석하라.

        반드시 아래 JSON 형식만 출력하라. 설명 문장은 추가하지 마라.
        {
          "signal": "긍정 | 중립 | 부정 중 하나",
          "summary": ["3개 이내의 핵심 요약"],
          "risks": ["3개 이내의 리스크 요약"],
          "actionItems": ["3개 이내의 실천 가능한 행동 제안"]
        }
        ...
        규칙:
        - 숫자 해석은 간결하고 과장 없이 작성한다.
        - 투자 수익을 보장하는 표현은 금지한다.
        - 초보 투자자도 이해할 수 있는 쉬운 한국어로 작성한다.
        """.trimIndent()
    }
